...
export default class TextFormAddOn extends AddOn {
  static _name: string = "textform";
  ...
  static create(node: HTMLElement, rawValue: any): Node {
    ...
    area.addEventListener("blur", (e) => {
      ...
      const readOnly = area.closest(".ql-disabled");
      if (!readOnly && area.value !== "") {
        area.placeholder = area.value;
        area.setAttribute("data-placeholder", area.value);
        area.value = "";
      }
    });
  ...
  static handleInsert(quill: any): void {
    ...
    quill.insertEmbed(range.index, "textform", {
      width: "100px",
      placeholder: "텍스트",
      size: fontSize,
    });
