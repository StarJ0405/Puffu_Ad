...
class _VendorRequester {
  instance;

  constructor() {
    // axios.defaults.withCredentials = true;
    this.instance = axios.create({
      baseURL: `${origin}/vendor`,
      headers: {
    ...
    this.instance.interceptors.request.use(async (config) => {
      ...
      if (typeof window === "undefined") {
        ...
        jwt = cookieStore.get(Cookies.VENDOR_JWT)?.value;
      } else {
        jwt = new ReactCookies().get(Cookies.VENDOR_JWT, getCookieOption());
      }
...
let vendorRequester: _VendorRequester;

if (process.env.NEXT_PUBLIC_DEV) {
  vendorRequester = new _VendorRequester();
} else {
  if (typeof window === "undefined") {
    if (!(global as any).vendorRequesterInstance) {
      (global as any).vendorRequesterInstance = new _VendorRequester();
    }
    vendorRequester = (global as any).vendorRequesterInstance;
  } else {
    if (!(window as any).vendorRequesterInstance) {
      (window as any).vendorRequesterInstance = new _VendorRequester();
    }
    vendorRequester = (window as any).vendorRequesterInstance;
  }
}

export { vendorRequester };